...
const Post = () => {
  ...
  const handleToggleStatus = async (postid, currentStatus) => {
    if (!isAdmin) {
      toast.error("Chỉ admin mới có quyền duyệt bài viết");
      return;
    }
    ...
    try {
      ...
      toast.success(
        nextStatus === 1
          ? "Bài viết đã được bật duyệt"
          : "Bài viết đã được gỡ duyệt",
      );
      setRefresh(true);
    } catch (error) {
      error.response
    ...
    <div className="p-3 text-xs md:text-base w-full">
      ...
      <div className="w-full mt-4 border rounded bg-white shadow-lg overflow-hidden">
        <div className=" flex p-3 font-semibold items-center bg-primary text-white">
          ...
          {isAdmin && <div className="w-1/12 text-center">Duyệt</div>}
            ...
            {search(data).map((p, i) => {
              ...
              return (
                <div className="w-full flex p-1 gap-1 items-center" key={i}>
                    ...
                    <div className="w-1/12 text-center flex justify-center items-center">
                      <div
                        className={`w-12 h-6 rounded-full relative ${
                          p.status === 1 ? "bg-primary" : "bg-slate-400"
                        }`}
                      >
                        <button
                          onClick={() => handleToggleStatus(p._id, p.status)}
                          disabled={st.id === 2}
                          className={`w-5 h-5 bg-white rounded-full absolute top-0.5 duration-400 transition-transform flex items-center justify-center ${
                            p.status === 1 ? "right-1" : "left-1"
                          }`}
                          title={p.status === 1 ? "Tắt duyệt" : "Bật duyệt"}
                        >
                  ...
                  <div className="w-1/12 flex justify-around gap-1 items-center">
                    {(isAdmin ||
                    ...
                    {(isAdmin || p.userId === currentUser._id) && (
                      <button
                        className={`p-1.5 ${
                          st.id === 2
