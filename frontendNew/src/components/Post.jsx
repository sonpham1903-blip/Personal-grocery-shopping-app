  ...
  return (
    <div className="px-2.5 mb-4">
      <div className="h-full bg-white border solid border-[#FBFAFA] shadow rounded-xl overflow-hidden">
        <Link
          to={`/posts/${props.id}`}
          className="block overflow-hidden w-full h-px pt-[60%] relative"
        >
          <img
            src={props.picNews}
            alt=""
        ...
        <div className="my-3 px-1.5">
          ...
          <div className="flex justify-between mt-4">
            <span className="text-gray-600 text-sm mb-2.5">
              {new Date(props.date).toLocaleString()}
            </span>
            <Link
              to={`/posts/${props.id}`}
              className="text-[#D7480E] relative pr-5 arrow-news"
            >
              Chi tiết
            </Link>
          </div>
...
const Posts = () => {
  ...
  const approvedPosts = data.filter((post) => post.status === 1);
  const hot =
    approvedPosts.length > 4 ? approvedPosts.slice(0, 4) : approvedPosts;

  return (
    <div className="max-w-screen-xl mx-auto px-2">
      <div className="flex justify-between">
        <h2 className="text-lg font-bold mb-3.5">TIN TỨC & SỰ KIỆN</h2>
        <Link to="/posts" className="font-medium text-lg text-[#D7480E]">
          Xem tất cả
        </Link>
      </div>
