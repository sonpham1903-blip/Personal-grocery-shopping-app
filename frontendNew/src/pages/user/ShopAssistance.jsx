...
const ShopAssistance = () => {
  ...
  const [messages, setMessages] = useState([
    {
      ...
      quickReplies: ["2 người ăn", "4 người ăn", "6 người ăn", "Ăn cơm văn phòng"]
    }
  ]);
  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      const res = await ktsRequest.post("/assistant/chat", { messages: history });
      ...
      if (res.data?.success) {
        ...
        if (status === "complete") {
          ...
          toast.success("💡 Đã lên thực đơn & chuẩn bị nguyên liệu thành công!");
        }
      } else {
  ...
  const handleAddSingleItem = (product, quantity) => {
    ...
    dispatch(addToCartServer({ productId: product._id, quantity }, currentUser));
  ...
  const handleAddAllToCart = async () => {
    ...
    try {
      ...
      toast.success("🎉 Tuyệt vời! Toàn bộ combo nông sản đã được đưa vào giỏ hàng!");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-gradient-to-br from-green-50 via-emerald-50 to-green-200">
      ...
      {/* Main Section */}
      <main className="max-w-screen-xl w-full mx-auto px-4 py-8 flex-grow">
        ...
        <div className="bg-gradient-to-r from-green-600 to-emerald-700 rounded-2xl p-6 md:p-8 text-white shadow-lg mb-8 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
          <div>
            ...
            <p className="text-green-100 mt-2 text-sm md:text-base">
              Lên thực đơn hoàn hảo, cân đối dinh dưỡng và tối ưu ngân sách của gia đình bạn.
            </p>
          </div>
          ...
          <div className="lg:col-span-7 flex flex-col bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 h-[650px]">
            ...
            <div className="bg-green-700 px-6 py-4 flex items-center justify-between text-white border-b border-green-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-sm md:text-base">Trợ Lý KTS Crop</h3>
                  <p className="text-xs text-green-200">Luôn sẵn sàng hỗ trợ bạn</p>
                </div>
              </div>
              <button 
                onClick={() => {
                  setMessages([
                    {
                      ...
                      quickReplies: ["4 người ăn", "Ngân sách 150k", "Thịt ba chỉ heo"]
                    }
                  ]);
                  setMealParams({ pax: null, budget: null, preference: null });
                  setComboProducts([]);
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/50">
              {messages.map((msg, index) => (
                <div key={index} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[85%] rounded-2xl px-4 py-3 shadow-sm transition-all duration-300 ${
                    msg.sender === "user" 
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100 space-y-4">
              <h3 className="text-lg font-bold text-gray-800 border-b pb-3 flex items-center justify-between">
                📊 Thông Số Bữa Ăn 
                <span className="text-xs text-gray-400 font-normal">Cập nhật tự động</span>
              </h3>
              
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-center">
                  <span className="text-xl">👥</span>
                  <p className="text-xs text-blue-700 mt-1 font-medium">Số Người</p>
                ...
                <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-3 text-center">
                  <span className="text-xl">💰</span>
                  <p className="text-xs text-yellow-700 mt-1 font-medium">Ngân Sách</p>
                  <p className="text-base font-bold text-gray-800 mt-0.5">
                ...
                <div className="bg-purple-50 border border-purple-100 rounded-xl p-3 text-center">
                  <span className="text-xl">🥗</span>
                  <p className="text-xs text-purple-700 mt-1 font-medium">Sở Thích</p>
                  <p className="text-sm font-bold text-gray-800 mt-0.5 truncate" title={mealParams.preference || ""}>
              ...
              <div className="space-y-1.5 pt-2">
                <div className="flex justify-between text-xs font-semibold text-gray-500">
                  ...
                  <span>
                    {((Boolean(mealParams.pax) + Boolean(mealParams.budget) + Boolean(mealParams.preference)) / 3 * 100).toFixed(0)}%
                  </span>
                </div>
                <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                  <div 
                    className="bg-green-600 h-full transition-all duration-500" 
                    style={{ width: `${(Boolean(mealParams.pax) + Boolean(mealParams.budget) + Boolean(mealParams.preference)) / 3 * 100}%` }}
                  ></div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100 space-y-6">
                ...
                <div className="space-y-3 bg-gray-50/50 p-4 rounded-xl border border-gray-100">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-semibold text-gray-600">Tổng chi phí thực tế:</span>
                    <span className="text-lg font-bold text-green-700">{vnd(totalPrice)}</span>
                  </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex justify-between text-[11px] text-gray-500 font-medium">
                        ...
                        <span className="flex items-center gap-1">
                          {budgetStatus === "over" && <span className="text-red-600">⚠️ Vượt ngân sách!</span>}
                          {budgetStatus === "within" && <span className="text-green-600">✅ Khớp ngân sách!</span>}
                          {budgetStatus === "under" && <span className="text-yellow-600">💡 Dưới ngân sách!</span>}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-3 divide-y divide-gray-100">
                    {comboProducts.map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center pt-3 first:pt-0">
                        <div className="flex items-center gap-3">
                          <img 
                            src={item.product?.thumbnail || item.product?.imgs?.[0] || "https://via.placeholder.com/60.png"} 
                            alt={item.product?.productName}
                            className="w-12 h-12 object-cover rounded-lg border border-gray-100 shadow-sm"
                          />
                          <div>
                            <p className="text-xs font-semibold text-gray-800 line-clamp-1">{item.product?.productName}</p>
                            <p className="text-[11px] text-gray-400">SL: {item.quantity} | {vnd(item.product?.currentPrice)}</p>
                          </div>
                        </div>
                        <button
                          onClick={() => handleAddSingleItem(item.product, item.quantity)}
              ...
              <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {alternativeProducts.map((prod, idx) => (
                    <div key={idx} className="bg-gray-50/50 rounded-xl p-2.5 border border-gray-100 flex flex-col justify-between hover:shadow-md transition-shadow">
                      <div>
                        <img 
                          src={prod.thumbnail || prod.imgs?.[0] || "https://via.placeholder.com/120.png"} 
                          alt={prod.productName}
                          className="w-full h-24 object-cover rounded-lg mb-2"
                        />
                        <p className="text-[11px] font-bold text-gray-800 line-clamp-1">{prod.productName}</p>
                        <p className="text-[10px] text-green-700 font-semibold mt-0.5">{vnd(prod.currentPrice)}</p>
                      </div>
                      <button
